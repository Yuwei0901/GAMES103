...
public class Rigid_Bunny_by_Shape_Matching : MonoBehaviour
{
	...
	Vector3[] P = { new Vector3(0, 0.01f, 0), new Vector3( 2, 0, 0) };  // Points of walls. 
	Vector3[] N = { new Vector3(0, 1,     0), new Vector3(-1, 0, 0) };  // Norm of walls.
    ...
    void Start()
    {
        ...
        V = new Vector3[mesh.vertices.Length]; // velocity of verticles
        X = mesh.vertices;					   // 
        Q = mesh.vertices;					   // 
	...
	void Collision(float inv_dt)
	{
		...
		for (int i = 0; i < X.Length; i++)
        {
            for (int n = 0; n < N.Length; n++)
            {
				...
				float phi = Vector3.Dot(c + R.MultiplyPoint(Q[i]) - P[n], N[n]);
				if(phi < 0)
				{
					Vector3 x = X[i];
					X[i] -= phi * N[n];
					float alpha = 1.0f; //stiffness coefficient
					float k     = 20000.0f; // penalty strength

					//update v when colleision 
					//???
					//V[i] += alpha * (X[i] - x) / dt + dt * k * Mathf.Abs(phi) * N[n] / m;
					V[i] += dt * k * Mathf.Abs(phi) * N[n] / m;
